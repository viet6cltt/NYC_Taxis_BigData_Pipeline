...
kubectl logs -n $NAMESPACE job/superset-add-trino-db 2>/dev/null | tail -20 || echo "❌ Init job not found or not run yet"
...
echo "  4. Check Admin -> Databases -> should see 'Trino - NYC Taxi Lakehouse'"
echo ""
echo "Run Trino query via Superset:"
echo "  SQL Lab -> SELECT * FROM delta.silver_nyc_taxi.trip_lifecycle WHERE status = 'completed' LIMIT 5"
echo ""
